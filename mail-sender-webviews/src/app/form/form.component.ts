...
@Component({
    ...
    styleUrls: [
        './form.component.scss',
        '../mail-form/mail-form.component.scss',
    ],
})
export class FormComponent implements OnChanges {
    ...
    public submitMail() {
        const mail: MailData = {
            from: this.reactiveForm.get('userFrom')?.value,
            to: this.reactiveForm.get('usersTo')?.value,
            subject: this.reactiveForm.get('subject')?.value,
            body: this.reactiveForm.get('mailContent')?.value,
        };
    ...
    public isFormValid(): boolean | undefined {
        return (
            this.reactiveForm.get('usersTo')?.value.length > 0 &&
            this.reactiveForm.get('subject')?.valid &&
            this.reactiveForm.get('mailContent')?.valid
        );
    }
